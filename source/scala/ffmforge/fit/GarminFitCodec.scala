 ...
 *
 * This is the ONLY file in FFMForge permitted to import `com.garmin.fit`. Decoding captures EVERY message and field
 * generically into [[FitMessage]]s (nothing is dropped), and encoding rebuilds them through `Factory.createMesg` so the
 * fields FFMForge never interprets still survive the round-trip.
 */
final class GarminFitCodec extends FitCodec {
  ...
  def encode(file: FitFile): Array[Byte] = {
    val tmp     = Files.createTempFile("ffm-forge-", ".fit")
