...
object FitEditor {
  ...
  def open(id: String, file: FitFile): EditorOpenResponse = {
    ...
    EditorOpenResponse(
      ...
      sensors = FitSummary.sensors(file),
  ...
  def rows(
    ...
    val selected = file.messages.zipWithIndex.filter { case (message, _) =>
      FitMetadata.messageName(message.globalNum) == requestedType
    }.toVector
  ...
  private def rowOf(
    ...
    EditorRecordRow(
      ...
      fields = FitMetadata
        .sortFields(messageType, message.fields.filterNot(_.num == Rec.Timestamp))
        .map(fieldCell(messageType, message, _)),
      issueIds = issueIds,
    )
  }

  private def fieldCell(messageType: String, message: FitMessage, field: RawField): EditorCell =
    EditorCell(
      FitMetadata.fieldName(message.globalNum, messageType, field.num),
      FitMetadata.formatField(message.globalNum, messageType, message, field),
      field.firstNumeric,
    )
  ...
  private def normalizeMessageType(messageType: String): String =
    messageType.trim.toLowerCase match {
      case ""                                 => "record"
      case "records"                          => "record"
      case "sensor_settings"                  => "sensor"
      case "sensor settings"                  => "sensor"
      case other if other.startsWith("mesg_") => other
      case other                              => other
    }
