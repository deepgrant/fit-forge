...
import fitforge.http.AwsS3Support

final class S3FitStoreSpec extends AnyFunSuite with Matchers with BeforeAndAfterAll {

  private implicit val system: ActorSystem = ActorSystem("s3-fit-store-spec")
  ...
  override def afterAll(): Unit = Await.result(system.terminate(), 30.seconds): Unit
